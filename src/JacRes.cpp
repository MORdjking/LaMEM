...
PetscErrorCode JacResCreateData(JacRes *jr)
{
	...
	ierr = DMCreateGlobalVector(fs->DA_CEN, &jr->dc); CHKERRQ(ierr);
...
PetscErrorCode JacResGetResidual(JacRes *jr)
{
	...
	START_STD_LOOP
	{
		...
		if (jr->ctrl.actDike)
		{
			...
			if (jr->ctrl.var_M)
			{
				ierr = GetDikeContrVarM(jr, svCell->phRat, jr->surf->AirPhase, dikeRHS, i - sx, j - sy); CHKERRQ(ierr);
			}
			else
			{
				ierr = GetDikeContr(jr, svCell->phRat, jr->surf->AirPhase, dikeRHS, y_c, j - sy); CHKERRQ(ierr);
			}
